...
export class Graph3DCore {
    ...
    init(container, config = {}) {
        ...
        this.graph = ForceGraph3D()(container)
            ...
            .nodeColor(node => node.currentColor || this.nodeColorMap.get(node.id) || node.color || '#666')
            .nodeVal(node => this.nodeSizeMap.get(node.id) || node.val || 1)
            .nodeOpacity(0.9)
            .nodeResolution(16)
            .linkColor(link => this.linkColorMap.get(`${link.source.id || link.source}-${link.target.id || link.target}`) || 'rgba(150, 150, 150, 0.5)')
            .linkWidth(link => this.linkWidthMap.get(`${link.source.id || link.source}-${link.target.id || link.target}`) || 0.1)
    ...
    updateNodeColors(colorMap) {
        ...
        // Update node colors in the data
        const currentData = this.graph.graphData();
        currentData.nodes.forEach(node => {
            if (colorMap.has(node.id)) {
                node.currentColor = colorMap.get(node.id);
                node.color = colorMap.get(node.id);
            }
        });
        
        // Force the graph to update colors
        this.graph.nodeColor(node => node.currentColor || this.nodeColorMap.get(node.id) || node.color || '#666');
    ...
    updateNodeSizes(sizeMap) {
        ...
        // Update node sizes in the data
        const currentData = this.graph.graphData();
        currentData.nodes.forEach(node => {
            if (sizeMap.has(node.id)) {
                node.val = sizeMap.get(node.id);
            }
        });
        
        // Force the graph to update sizes
        this.graph.nodeVal(node => this.nodeSizeMap.get(node.id) || node.val || 1);
    ...
    updateLinkColors(colorMap) {
        ...
        // Force the graph to update link colors
        this.graph.linkColor(link => {
            const linkId = `${link.source.id || link.source}-${link.target.id || link.target}`;
            return this.linkColorMap.get(linkId) || 'rgba(150, 150, 150, 0.5)';
        });
    ...
    updateLinkWidths(widthMap) {
        ...
        // Force the graph to update link widths
        this.graph.linkWidth(link => {
            const linkId = `${link.source.id || link.source}-${link.target.id || link.target}`;
            return this.linkWidthMap.get(linkId) || 0.1;
        });
        ...
        if (enabled) {
            this.graph
                ...
                .nodeThreeObject(node => {
                    ...
                    // Get the actual color from the node color map or use current color
